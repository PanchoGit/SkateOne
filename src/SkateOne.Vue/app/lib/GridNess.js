 ...
 *	<>selectRow({[array of Ness.Grid.Pojo.Row]})
...
Ness.Grid.EventClick = function(idGrid,rowIndex){
	var instance = Ness.Elements.getByID(idGrid);
	var linea = instance.dataTable.rows[rowIndex];
	instance.onClick(linea);
	instance.selectRow();
}
Ness.Grid.prototype.clearRowSelek = function(){
	$("#"+ this.id).parent().find("td").removeClass("nessRowSelek");
}
Ness.Grid.prototype.selectRow = function(){
	this.clearRowSelek();
	$(".nessRowOver").addClass("nessRowSelek");
}
...
Ness.Grid.prototype.orderBy = function(colSort){
	Ness.ColSort = colSort; //variable global
	var colType = this.dataTable.cols[Ness.ColSort].type;
	var row2Order = this.dataTable.rows.slice();
...
Ness.Grid.prototype._sortNumberFunc = function(a, b){
    return b[Ness.ColSort].data - a[Ness.ColSort].data;
}
Ness.Grid.prototype._sortStringFunc = function(a, b){
    var valor1 = a[Ness.ColSort].data;
    var valor2 = b[Ness.ColSort].data;
    if (valor1 > valor2)
        return 1;
    if (valor1 < valor2)
        return -1;
